...
export default class IpLimitEdit extends Vue {
  ...
  mounted(this: any) {
    ...
    IpLimitService.get({ IP: this.itemObject.IP }).then((response: any) => {
      this.sharedResponse(response, {
        useSuccessMessage: false,
        callback: () => {}
      })
      let itemArray = response.data.Data
      this.forms.IP = itemArray.IP
      this.forms.Memo = itemArray.Memo
      this.forms.IsValid = itemArray.IsValid
      this.forms.ExpiredTime = itemArray.ExpiredTime
    })
  }
  @Emit('closeDialog')
  close() {
  ...
  async submit(this: any): Promise<void> {
    ...
    if (allOK) {
      ...
      IpLimitService.edit(this.forms)
        .then(response => {
          this.sharedResponse(response, {
            useBack: false,
