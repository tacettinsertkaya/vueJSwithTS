...
// 三方平台銀行-新增
import SubAdd from '@/views/tPlatformBank/Add.vue'
// 三方平台銀行-編輯
import SubEdit from '@/views/tPlatformBank/Edit.vue'
// 三方平台銀行-刪除
...
// 三方平台銀行-欄位
import TPlatformBankFields from '@/views/tPlatformBank/model/TPlatformBankFields'
// 共用模組
...
export default class TPlatformBankIdex extends Vue {
  dataSource: any = []
  fields = new TPlatformBankFields().getFields()
  filter = new RequestTPlatformBankGetMany({
    TPlatformID: -1,
    BankID: -1,
    ThirdPartyBankCode: '',
    PageIndex: 1,
  ...
  recordCount = 0 // 資料筆數
  dialog = {
    add: { name: 'add', title: '新增銀行', show: false },
    edit: { name: 'edit', title: '编辑銀行', show: false, itemObject: null }
  }
  other = {
    SelectAll: []
  }
  ...
  dataBind() {
    ...
    // 取得全部
    TPlatformBankService.getMany(this.filter)
      .then((response: any) => {
      ...
      .catch((error: any) => {
        console.log('撈取三方平台銀行發生例外', error)
        this.listLoading = false
      })
  }
